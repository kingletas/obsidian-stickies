...
```bash
git clone https://github.com/kingletas/obsidian-stickies && cd obsidian-stickies
npm ci
npm test
```
...
**Develop against a throwaway vault, never your real one.** The annotation store is a single JSON file, and a bug in the writer loses every sticky in the vault at once.
...
`test/reserved.test.cjs` walks the view and panel prototypes against Obsidian's own member lists. It exists because this plugin lost three releases to a method named `open()` shadowing `View.open()` — the surface rendered nothing while every other test passed. **It is not a formality.**

`test/anchor.test.cjs` covers the part that is genuinely hard: keeping a sticky attached to a passage while the note is edited around it.
